...
def acquire_stock_data(path):
    ...
    with open(path, 'r', newline='', encoding = 'utf-8-sig') as tickers:
        ...
        stock_close_data_concat_diff = stock_close_data[0]
        ...
        for l in range(1, len(stock_close_data)):
            stock_close_data_concat_diff = pl.concat([stock_close_data_concat_diff, stock_close_data[l].select(pl.selectors.by_index([2]))], how = "horizontal") # only diff returns at index 2

        # Fixing columns in the concat diff dataframe 
        ...
        export_values_to_csv('stock_data_concat.csv', stock_close_data_concat_diff)
        export_values_to_csv('stock_data_average.csv', stock_close_data_average_diff)

        # Returning the average of the stocks, which is the only one used in any calculations anyway
        return stock_close_data_average_diff
